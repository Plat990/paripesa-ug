...
function denyWager(message){verdictView.textContent=message;wagerInput.classList.remove('rl-deny-0965382f');void wagerInput.offsetWidth;wagerInput.classList.add('rl-deny-0965382f');}
...
rolling=true;rollBtn.disabled=true;clearFlash();wagerInput.classList.remove('rl-deny-0965382f');purse=round2(purse-wager);savePurse();renderPurse();var target=currentTarget();var prizeRate=payoutFor(target);verdictView.textContent='The die is rolling…';dieView.classList.add('rl-spin-0965382f');scrambleTimer=window.setInterval(function(){dieView.textContent=String(Math.floor(Math.random()*100)+1);},SCRAMBLE_TICK_MS);window.setTimeout(function(){settleRoll(wager,target,prizeRate);},SCRAMBLE_MS);}
...
var brandEl=document.querySelector('.rl-brand-0965382f');var echoEl=grab('rl-echo');if(brandEl&&echoEl){echoEl.textContent=brandEl.textContent.replace(/\s+/g,' ').trim();}
